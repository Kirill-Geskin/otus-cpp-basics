...
void getUserSolution(std::string userName) // спрашиваем хочет ли пользователь мен€ть диапозон чисел дл€ игры. 
{
    char userSolution{ 0 };
...
void helloFunc(std::string userName) // ≈сли, пользователь отказадс€ мен€ть диапозон, передаем стандартный диапазон 0 и 99.
{
